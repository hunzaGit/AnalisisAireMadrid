...
zona2y3 = ['008','011','039','047','049','050','056']
...
resultados_formateados_zona2y3 = resultados_formateados.filter(lambda valor: valor[2] in zona2y3)
...
resultados_formateados_zona2y3 = resultados_formateados_zona2y3.map(lambda valor: (valor[0], valor[1]))
...
resultados_formateados_zona2y3 = resultados_formateados_zona2y3.groupByKey().map(lambda x : (x[0],list(x[1])))
resultados_formateados_zona2y3 = resultados_formateados_zona2y3.map(lambda tupla: tupla[0] + ';' + ';'.join(str(d) for d in tupla[1]))
resultados_formateados_zona2y3.saveAsTextFile("medicionesdiarias_zona2y3.csv")
